...
from nextcord import Interaction, SelectOption, ui, TextInputStyle, ChannelType #type: ignore
from nextcord.ui import Button, View, Modal, TextInput, RoleSelect, ChannelSelect, Select #type: ignore
import aiosqlite #type: ignore
import re
from nextcord.utils import utcnow #type: ignore
...
async def setup_command(inter: Interaction):
    # Define the SetupView class inside the slash command
    class SetupView(View):
        def __init__(self):
            ...
            self.assistant_manager_role_select = RoleSelect(
            ...
            self.manager_role_select = RoleSelect(
        ...
        async def assistant_manager_role_select_callback(self, interaction: Interaction):
            selected_role_id = self.assistant_manager_role_select.values[0]  # Get the selected role ID
            print(f'Selected Assistant Manager Role ID: {selected_role_id}')  # Debug print
            
            async with aiosqlite.connect(DATABASE_PATH) as db:
                async with db.cursor() as cursor:
                    await cursor.execute(
                        "UPDATE config SET assistant_manager_role_id = ? WHERE guild_id = ?",
                        (selected_role_id, interaction.guild.id)
                    )
                    await db.commit()
                    print("Updated Assistant Manager Role ID in the database.")  # Debug print
            

        async def manager_role_select_callback(self, interaction: Interaction):
            selected_role_id = self.manager_role_select.values[0]  # Get the selected role ID
            print(f'Selected Manager Role ID: {selected_role_id}')  # Debug print
            ...
            async with aiosqlite.connect(DATABASE_PATH) as db:
                async with db.cursor() as cursor:
                    ...
                    await db.commit()
                    print("Updated Manager Role ID in the database.")  # Debug print
            

        async def channel_select_callback(self, interaction: Interaction):
            selected_channel_id = self.channel_select.values[0]  # Get the selected channel ID
            print(f'Selected Channel ID: {selected_channel_id}')  # Debug print
            
            async with aiosqlite.connect(DATABASE_PATH) as db:
                async with db.cursor() as cursor:
                    await cursor.execute(
                        "UPDATE config SET channel_id = ? WHERE guild_id = ?",
                        (selected_channel_id, interaction.guild.id)
                    )
                    await db.commit()
                    print("Updated Channel ID in the database.")  # Debug print
            

    # Define the SetRosterModal class inside the slash command
    class SetRosterModal(Modal):
        ...
        async def callback(self, interaction: Interaction):
            print(f'Roster input: {self.roster_input.value}')  # Debug print
            ...
            async with aiosqlite.connect(DATABASE_PATH) as db:
                async with db.cursor() as cursor:
                    ...
                    await db.commit()
                    print("Updated Roster in the database.")  # Debug print
            

    view = SetupView()
    await inter.response.send_message("Please select the Assistant Manager role, Manager role, and a channel from the dropdowns below, and then set the roster if needed.", view=view, ephemeral=False)
...
async def sign(interaction: nextcord.Interaction, player: nextcord.Member):
    ...
    manager_id = int(config['manager_role_id'])  # Ensure IDs are integers
    assistant_manager_role_id = int(config['assistant_manager_role_id'])
    ...
    user_roles = [role.id for role in interaction.user.roles]
    print(f"User Roles: {user_roles}")  # Debugging line
    if user.id != manager_id or assistant_manager_role_id  in user_roles:
    ...
    offer_embed = nextcord.Embed(
        title="Franchise Signing",
    ...
    offer_embed.set_author(name=f"{interaction.guild.name} Transactions", icon_url=server_icon_url)
    offer_embed.set_thumbnail(url=get_emoji_image_url(team_emoji))
    offer_embed.set_footer(text=f"{user.name}", icon_url=f"{user.avatar.url}")
    ...
    async def accept_callback(interaction: nextcord.Interaction):
        ...
        update_embed.set_author(name=f"{server_name} Transactions", icon_url=server_icon_url)
        update_embed.set_thumbnail(url=get_emoji_image_url(team_emoji))
        update_embed.set_footer(text=f"{user.name}", icon_url=f"{user.avatar.url}")
...
class TicketDropdown(Select):
    def __init__(self):
        options = [
            SelectOption(label="Report a Member", description="Report Someone", emoji="❗"),
            SelectOption(label="General Support", description="General Support", emoji="❔"),
            SelectOption(label="Apply For Media Maker", description="Apply to Join the GFX team", emoji="🎨"),
            SelectOption(label="Apply For Staff", description="Apply to be Moderator", emoji="🛡️"),
        ]
...
async def create_ticket_channel(interaction: Interaction):
    async with aiosqlite.connect('tickets.db') as db:
        await db.execute('INSERT INTO tickets (ticket_number) VALUES (NULL)')
        await db.commit()
        
        cursor = await db.execute('SELECT id FROM tickets ORDER BY id DESC LIMIT 1')
        row = await cursor.fetchone()
        ticket_number = f"{row[0]:04d}"  # Format the ticket number to be 4 digits (e.g., 0001)
        await db.execute('UPDATE tickets SET ticket_number = ? WHERE id = ?', (ticket_number, row[0]))
        await db.commit()

    guild = interaction.guild
    category_id = 1261080210577555650  # Specify a category ID
    ...
    embed = nextcord.Embed(title="Ticket Created", description=f"Ticket ID: {ticket_number}")
    await ticket_channel.send(embed=embed, view=TicketChannelView())
...
async def send_ticket_panel(channel):
    logging.info('ticket pannel sent')
    embed = nextcord.Embed(
        title="Ticket Creation",
        description="Select an option from the dropdown to create a ticket.",
        colour=0x871a3b
    )

    embed.set_author(
        name="MRF - Modern Football League S1",
        icon_url="https://cdn.discordapp.com/attachments/1266386138030145619/1272993601772720170/tps_x_league_1-pdf_1.png?ex=66bcff2d&is=66bbadad&hm=340739d15b78e6289911da21f055975dd682f885fed3a985c5b38fac275e561b&"
    )

    embed.set_image(
        url="https://cdn.discordapp.com/attachments/1221563878824087675/1272995796387889343/New_Project_48.png?ex=66bd0138&is=66bbafb8&hm=275b8984f2740c10e799eca3f9a629057bd41c47251ffa860d3a6a9d4bf8dad1&"
    )

    embed.set_thumbnail(
        url="https://cdn.discordapp.com/attachments/1266386138030145619/1272993601772720170/tps_x_league_1-pdf_1.png?ex=66bcff2d&is=66bbadad&hm=340739d15b78e6289911da21f055975dd682f885fed3a985c5b38fac275e561b&"
    )
